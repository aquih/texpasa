...
class AccountAsset(models.Model):
    ...
    def copiar_activo(self):
        if not self.company_id.compania_relacionada_id:
            raise UserError("Aún no se ha configurado una compañía fiscal relacionada")

        if self.sinc_id > 0:
            raise UserError("Este activo ya fue creado en la compañía relacionada: '" + self.company_id.compania_relacionada_id.name + "'")

        if self.activo_padre_id and self.id != self.activo_padre_id.id:
            if self.activo_padre_id.sinc_id == 0:
                raise UserError("El activo padre '" + self.activo_padre_id.name + "' aún no ha sido creado en la compañía relacionada: '" + self.company_id.compania_relacionada_id.name + "'")

        modelo_activo_relacionado_id = self.env['account.asset'].sudo().search([('company_id', '=', self.company_id.compania_relacionada_id.id), ('name', '=', self.model_id.name)], limit=1)
        if not modelo_activo_relacionado_id:
            raise UserError("No existe un 'Modelo de activo' con el nombre '" + self.model_id.name + "' en la compañía relacionada: '" + self.company_id.compania_relacionada_id.name + "'")
